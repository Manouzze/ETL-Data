...
df_filtered_keto = df.filter(
    ...
    (col("energy-kcal_100g") < 300)  # Choisissez un seuil en fonction de votre définition de "élevé en énergie"
)
...
for day in range(1, 2):
    ...
    products_data.extend([
        (plat1, 'vegetalien', plat1_quantity, 'plats', plat1_glucides, plat1_sodium, 2),
        (dessert1, 'vegetalien', dessert1_quantity, 'desserts', dessert1_glucides, dessert1_sodium, 2),
        (plat2, 'vegetalien', plat2_quantity, 'plats', plat2_glucides, plat2_sodium, 2),
        (dessert2, 'vegetalien', dessert2_quantity, 'desserts', dessert2_glucides, dessert2_sodium, 2),
    ])
    ...
    CREATE TABLE IF NOT EXISTS ProduitsOpenFoodFacts (
        ...
        glucides FLOAT,
        sodium FLOAT,
        programme_alimentaire_id INTEGER
    );
    """)

    # Insérer les données générées dans la table existante ProduitOpenFoodFacts avec la valeur 'plats' ou 'desserts' pour la colonne food_groups
    cursor.executemany("""
